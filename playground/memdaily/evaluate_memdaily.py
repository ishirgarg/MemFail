...
class BaseHarness:
    ...
    def __init__(
        ...
        client_kwargs: Dict[str, Any] = {"api_key": api_key}
        if answer_base_url:
            client_kwargs["base_url"] = answer_base_url
        self.answer_client = OpenAI(**client_kwargs)
...
class Mem0Harness(BaseHarness):
    ...
    def __init__(
        ...
        if not getattr(LLMBase, "_memdaily_filters_none_params", False):
            original_get_common_params = LLMBase._get_common_params

            def _get_common_params_without_none(self, **kwargs):
                params = original_get_common_params(self, **kwargs)
                return {key: value for key, value in params.items() if value is not None}

            LLMBase._get_common_params = _get_common_params_without_none
            LLMBase._memdaily_filters_none_params = True
        ...
        llm_cfg_data: Dict[str, Any] = {"model": mem0_llm_model}
        if mem0_llm_base_url:
            llm_cfg_data["openai_base_url"] = mem0_llm_base_url
            # LiteLLM adapters like Vertex Claude reject requests that specify
            # both temperature and top_p; omit top_p on the OpenAI-compatible path.
            llm_cfg_data["top_p"] = None
        if mem0_llm_api_key:
            llm_cfg_data["api_key"] = mem0_llm_api_key
        llm_cfg = LlmConfig(provider=mem0_llm_provider, config=llm_cfg_data)
...
class SimpleMemHarness(BaseHarness):
    ...
    def __init__(
        ...
        work_dir: Path,
        answer_base_url: Optional[str] = None,
        simplemem_model: Optional[str] = None,
        simplemem_base_url: Optional[str] = None,
        simplemem_api_key: Optional[str] = None,
    ):
        super().__init__(
            llm_model=llm_model,
            api_key=api_key,
        ...
        internal_model = simplemem_model or llm_model
        internal_api_key = simplemem_api_key or api_key
        ...
        # The tracked SimpleMem checkout expects a local config.py. For benchmark
        # runs, inject a process-local config module instead of writing secrets to disk.
        benchmark_config = types.ModuleType("config")
        benchmark_config.OPENAI_API_KEY = internal_api_key
        benchmark_config.OPENAI_BASE_URL = simplemem_base_url
        benchmark_config.LLM_MODEL = internal_model
        benchmark_config.EMBEDDING_MODEL = "sentence-transformers/all-MiniLM-L6-v2"
        benchmark_config.EMBEDDING_DIMENSION = 384
        benchmark_config.EMBEDDING_CONTEXT_LENGTH = 512
        benchmark_config.ENABLE_THINKING = False
        benchmark_config.USE_STREAMING = True
        benchmark_config.USE_JSON_FORMAT = False
        benchmark_config.WINDOW_SIZE = 40
        benchmark_config.OVERLAP_SIZE = 2
        benchmark_config.SEMANTIC_TOP_K = retrieval_limit
        benchmark_config.KEYWORD_TOP_K = retrieval_limit
        benchmark_config.STRUCTURED_TOP_K = retrieval_limit
        benchmark_config.LANCEDB_PATH = "./lancedb_data"
        benchmark_config.MEMORY_TABLE_NAME = "memory_entries"
        benchmark_config.ENABLE_PARALLEL_PROCESSING = True
        benchmark_config.MAX_PARALLEL_WORKERS = 16
        benchmark_config.ENABLE_PARALLEL_RETRIEVAL = True
        benchmark_config.MAX_RETRIEVAL_WORKERS = 8
        benchmark_config.ENABLE_PLANNING = True
        benchmark_config.ENABLE_REFLECTION = True
        benchmark_config.MAX_REFLECTION_ROUNDS = 2
        benchmark_config.JUDGE_API_KEY = internal_api_key
        benchmark_config.JUDGE_BASE_URL = simplemem_base_url or "https://api.openai.com/v1/"
        benchmark_config.JUDGE_MODEL = internal_model
        benchmark_config.JUDGE_ENABLE_THINKING = False
        benchmark_config.JUDGE_USE_STREAMING = False
        benchmark_config.JUDGE_TEMPERATURE = 0.3
        sys.modules["config"] = benchmark_config
    ...
    def retrieve_memories(self, question: str) -> str:
        ...
        if not contexts:
            return ""
        contexts = contexts[: self.retrieval_limit]
...
def build_harness(
    ...
    if system == "simplemem":
        return SimpleMemHarness(
            llm_model=llm_model,
            api_key=api_key,
            retrieval_limit=retrieval_limit,
            work_dir=work_dir,
            answer_base_url=llm_base_url,
            simplemem_model=simplemem_model,
            simplemem_base_url=simplemem_base_url,
...
def run_system(
    *,
    ...
    answer_api_key = llm_api_key or api_key
    judge = Judge(api_key=judge_api_key or api_key, model=judge_model, base_url=judge_base_url)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="API key for answer generation. Defaults to OPENAI_KEY / OPENAI_API_KEY.",
    )
    parser.add_argument(
        "--judge-model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="OpenAI-compatible base URL for failure classification.",
    )
    parser.add_argument(
        "--judge-api-key",
        type=str,
        default=None,
        help="API key for failure classification. Defaults to OPENAI_KEY / OPENAI_API_KEY.",
    )
    parser.add_argument(
        "--num-memories",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="OpenAI-compatible base URL for mem0 memory operations.",
    )
    parser.add_argument(
        "--mem0-llm-api-key",
        type=str,
        default=None,
        help="API key for mem0 memory operations. Defaults to the answer-generation API key.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="LLM model for SimpleMem memory operations. Defaults to --llm-model.",
    )
    parser.add_argument(
        "--simplemem-base-url",
        type=str,
        default=None,
        help="OpenAI-compatible base URL for SimpleMem memory operations.",
    )
    parser.add_argument(
        "--simplemem-api-key",
        type=str,
        default=None,
        help="API key for SimpleMem memory operations. Defaults to the answer-generation API key.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    resolved_simplemem_model = resolve_model_name(args.simplemem_model) if args.simplemem_model else None
    ...
    if resolved_simplemem_model and resolved_simplemem_model != args.simplemem_model:
        print(f"Resolved simplemem model alias: {args.simplemem_model} -> {resolved_simplemem_model}")
    ...
    for system in systems:
        result = run_system(
            ...
            mem0_llm_base_url=args.mem0_llm_base_url,
            mem0_llm_api_key=args.mem0_llm_api_key,
            ...
            simplemem_model=resolved_simplemem_model,
            simplemem_base_url=args.simplemem_base_url,
            simplemem_api_key=args.simplemem_api_key,
            support_set_plan=support_set_plan or None,
        )
